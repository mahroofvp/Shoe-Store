...
import { selectedTotalQty, setOpenCart } from '../Redux/CartSlice'
    ...
    return (
        <>
            <header className={ 
                ...
                : 'fixed top-0 right-0 left-0 h-[9vh] flex items-center justify-center opacity-100 z-[101] blur-effect-theme'
            }>
                <nav className='flex items-center justify-between nike-container'>
                    ...
                    <ul className='flex items-center justify-center gap-2'>
                        ...
                        <li className='grid items-center'>
                            <button type='button' onClick={()=>dispatch(setOpenCart())} className=' border-none outline-none 
                active:scale-110 transition-all duration-300 relative'>
                    ...
                    text-slate-900 shadow shadow-slate-100 w-4 h-4
                     text-[0.65rem] leading-tight font-medium rounded-full
                     flex items-center justify-center cursor-pointer
                      hover:scale-110 transition-all duration-300  ${navState ? ' bg-slate-900 text-slate-100 shadow-slate-900  ':'bg-slate-100 text-slate-900 shadow-slate-100 '}`}>{totalQty}</div>
                            </button>
                        </li>
                    </ul>
                </nav>
